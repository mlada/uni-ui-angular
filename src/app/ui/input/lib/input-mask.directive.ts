...
import type { ChangeFn, MaskFn, TouchedFn } from '@uni/core';
...
@Directive({
  selector: 'input[formControlName][uniInputMask],input[formControl][uniInputMask]',
  standalone: true,
  providers: [
    {
...
export class InputMaskDirective implements ControlValueAccessor, OnInit {
  ...
  readonly mask = input.required<string>({ alias: 'uniInputMask' });
  readonly maskFrom = input<MaskFn>(DEFAULT_MASK_FN, { alias: 'uniInputMaskFrom' });
  readonly maskTo = input<MaskFn>(DEFAULT_MASK_FN, { alias: 'uniInputMaskTo' });
